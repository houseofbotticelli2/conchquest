...
- [ ] #79 Stop keeping the Railway Postgres public URL sitting in local `.env` files -- disable public networking on the Postgres service and use `railway connect` (Railway CLI) to tunnel in instead for one-off local dev/admin/debugging work; more setup than a static `DATABASE_URL`, but nothing sits exposed between sessions. **Before disabling public networking, confirm the deployed `conchquest-api` service's own `DATABASE_URL` variable references Postgres's private/internal URL** (a `${{Postgres.DATABASE_URL}}`-style reference resolving to `postgres.railway.internal`, not the public `*.proxy.rlwy.net` host) -- as of 2026-08-04, local `api/.env` was using the public proxy host (`zephyr.proxy.rlwy.net`), so check the deployed service isn't doing the same or disabling public networking will take production down, not just tighten local dev. In the meantime: never paste the public URL anywhere outside this session's chat (no public gists, tickets, etc.), and if it's ever suspected leaked, rotate it immediately from Railway's dashboard (Postgres service -> Variables -> regenerate)
- [ ] #83 `web/`'s custom domain is live (`www.conchquest.app`, apex `conchquest.app` forwards to it) -- home, privacy, terms, about, and shop pages now have real content (see #88/#89); `download` and `community-guidelines` are still placeholders (community-guidelines content is tracked separately as #75).
