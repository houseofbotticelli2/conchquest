...
- [ ] #86 `admin.conchquest.app` is now publicly reachable (deployed and DNS-live) but the earlier decision to keep the admin console VPN/Tailscale-restricted rather than publicly exposed was never implemented -- right now it's protected only by the cookie-session login + admin-role check, no network-level restriction. Decide whether that's acceptable long-term or whether to add the VPN/Cloudflare Access layer discussed earlier.
