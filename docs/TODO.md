...
- [ ] #60 Finish the password reset flow -- the actual flow is built and verified end-to-end (mobile's "Forgot password?" link on the login screen calls `resetPasswordForEmail`; `web/`'s `/reset-password` page detects the recovery session and updates the password; confirmed live with a throwaway account, including a real login with the new password). What's left: (1) `mobile/src/lib/supabase.ts`'s `WEB_APP_URL` is hardcoded to `http://localhost:4321` -- update it once `web/` is actually deployed (see #83); (2) Supabase's Auth -> URL Configuration -> Redirect URLs allowlist needs that real domain added -- confirmed directly that Supabase silently ignores any `redirectTo`/`redirect_to` not on that allowlist and falls back to the project's default Site URL (currently `http://localhost:3000`, a dead API-only URL) instead of erroring, which was the root cause of the original broken reset email
...
- [ ] #83 Deploy `web/` (the public marketing/download/legal/password-reset site) somewhere real with a real custom domain -- it currently only runs via `npm run dev` locally. Already has a `railway.json` + `serve`-based start script ready to go, same pattern as the API. Once it has a real domain: update `mobile/src/lib/supabase.ts`'s `WEB_APP_URL` (see #60) and add the domain to Supabase's Auth Redirect URLs allowlist. Also still needs actual content for the home/download/privacy/terms/community-guidelines pages -- currently placeholders (privacy/terms/community-guidelines content is tracked separately as #75).
