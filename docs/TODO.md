...
- [ ] #82 Deploy the admin console (`admin/`) somewhere real -- it currently only runs via `npm run dev` locally, never hosted. Needs a production static server (Vite only builds static files; `vite preview` isn't meant for production, so add `serve` or similar + a start script), a new Railway service with its Root Directory set to `admin`, and once it has a real origin, add that origin to `CORS_ALLOWED_ORIGINS` on the API service (currently only defaults to local-dev origins) and switch `admin/.env.local`'s `VITE_API_BASE_URL` back to the deployed API if it was pointed at it for testing. Given the earlier decision to keep the admin console VPN/Tailscale-restricted rather than publicly reachable, factor that into how (or whether) this gets exposed.
...
- [x] #85 Attach custom domains on Railway -- `conchquest.app` (GoDaddy) is split across two services in one project: `www.conchquest.app` -> `conchquest-web` (the apex CNAME approach was rejected by GoDaddy with "invalid name for '@'", so the working pattern is `www` as the real Railway custom domain + GoDaddy's domain-forwarding feature for the bare apex -> `www`), and `api.conchquest.app` -> `conchquest-api` (subdomain CNAME, no forwarding needed). Both have valid, issued SSL certs, confirmed live via `curl`. `admin.conchquest.app` is reserved for later, once #82 (admin's deployment) happens -- not registered yet. `mobile/src/lib/supabase.ts`'s `WEB_APP_URL` and `admin/src/lib/api.ts`'s default `API_BASE_URL` both updated to the new custom domains.
