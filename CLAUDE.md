...
- **Disposable test accounts have to be torn down on *both* sides.** Deleting the Supabase user is only half of it — the first authenticated API call already created a matching row in our own `users` table, and that row survives independently. It then keeps whatever the test created: `shell_finds` rows stay on the community map for real testers to see, and reports/blocks linger. This bit us: verification accounts from Aug 13-15 left 13 orphaned users and two **public** finds ("test find for moderation", "public test find") visible in production until 2026-08-17. Deleting from `users` cascades to finds/reports/blocks, so the app-side teardown is one statement — but nothing does it for you, and the Supabase side looks clean while it's still there. To audit: compare `select id from users` against the Admin API's user list (see `api/scripts/backup-auth.mjs`); anything in the former and not the latter is an orphan.
